...
{
    if(score>hiscorelist[9].score) return(1);
    else return(0);
}
    ...
    for(p=0;p<MAX_PLR;++p)
    {
        ...
        for(i=0;i<10;++i)
        {
            if(hiscorelist[i].lastplr==p)
            {
                ...
                gotoxy(((SCREENX-21)/2),18);cprintf("Player %d - enter name",(p+1));
        ...
        gotoxy(((SCREENX-22)/2),18);cclear(SCREENX-((SCREENX-22)/2));
        printhiscores();
    }
